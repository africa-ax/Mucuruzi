...
const OrderDetailView = (() => {
        ...
        ${isSeller && order.status === ORDER_STATUS.PENDING ? `
          <div class="card">
            ...
            <p class="text-muted text-sm mb-md">
              Ask the buyer to dial <strong>*800*${order.sellerTIN}#</strong> on their phone to get a purchase code.
            </p>
            <div class="form-group">
              <label class="form-label">Purchase Code (5 or 6 digits)</label>
              <input id="purchase-code" type="text" class="form-input"
                placeholder="Enter buyer's purchase code" maxlength="6"
  ...
  const _confirm = async (orderId) => {
    ...
    if (!code) { Toast.error('Please enter the purchase code.'); return; }
    if (!/^\d{5,6}$/.test(code)) { Toast.error('Purchase code must be 5 or 6 digits.'); return; }
