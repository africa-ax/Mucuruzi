...
const ProductListView = (() => {
          ...
          return `
            <div class="card product-card" style="padding:var(--space-md)">
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                <span class="badge badge-muted" style="font-size:0.62rem">${safe(p.rraItemCode)}</span>
    ...
    Modal.show({
      ...
      body: `
        <div style="background:var(--color-surface-2);border-radius:var(--radius-md);padding:var(--space-md);margin-bottom:var(--space-md)">
          <p style="font-weight:700;font-size:0.875rem">${safe(p.productName)}</p>
          <p class="text-muted text-xs">${safe(p.rraItemCode)} · ${safe(p.category)} · ${p.unit}</p>
        </div>
        <div class="form-group">
  ...
  const _submitListing = async (p, isUpdate) => {
    ...
    // Importer gets 'imported' source, producer gets 'produced'
    const source  = user.businessType === BUSINESS_TYPES.IMPORTER ? 'imported' : 'produced';
    ...
    Loader.hide();
    Toast.success(isUpdate ? 'Listing updated!' : `${safe(p.productName)} listed!`);
    await _load(user);
  };
