...
const App = (() => {
  ...
  const _renderView = (route) => {
    ...
    switch (route.view) {
      case 'auth':
        _tearDownLayout();
        // Auth screens are full screen — render into app-root directly
        const appRoot = document.getElementById('app-root');
        if (appRoot) {
          appRoot.className = ''; // remove app-main class
          appRoot.classList.remove('hidden');
        }
  ...
  const init = () => {
    ...
    AuthService.onAuthStateChanged(async (user) => {
      ...
      if (user) {
        ...
        // If currently on auth page after login/register → go to dashboard
        const hash = window.location.hash;
